...
```{r, fig.height=3, fig.cap= 'Count Trends for Three Organizations'}
motivation.plot 
```
...
```{r}
#As we describe the model, for convenience’s sake, let’s say we’re dealing with three lists: 1, 2, and 3. Obviously for our case we deal with nearly 15 lists, the model gets very complex. We actually end up fitting a model with 4 systems, but for the sake of simplicity 
```
