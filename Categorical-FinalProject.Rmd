...
## Introduction: Estimates of Killing in Casanare
...
## Review of Multiple Systems Estimation (Capture-recapture)
...
colnames <- c("Organization", "Total Captures", "Unique", "Type")

knitr::kable(contigency_table)
...
ggplot(casanare_wide, aes(x= factor(year), y= as.integer(n), fill = as.factor(data_lists)))+
...
Our estimates are primarily based on Poisson regression. As we describe the model, for convenience’s sake, let’s say we’re dealing with three lists: 1, 2, and 3. Obviously for our case we deal with nearly 15 lists, the model gets very complex. We actually end up fitting a model with 4 systems, but for the sake of simplicity 

Let’s further say that the number of violent incidents selected into a list or combination of lists is $m_{100}$,  $m_{010}$, $m_{001}$,$m_{110}$, $m_{101}$, $m_{001}$, and $m_{111}$.

Each of these cells is a count of cases. For example, $m_{100}$,is the number of cases in only list 1; $m_{110}$, is the number of cases that appear in lists 1 and 2, but not list 3—and so on.

The $m$‘s are called cell counts (as in a contingency table **DRAW CONTIGENCY TABLE HERE** ), and Poisson regression treats these cell counts—not the underlying individual cases data. The (log of the) expected cell count $m_{100}$ is a function of the other observed cell counts, as shown in the equation below.

$$\log(m_{000}) = a + \beta_1(m_{100}) + \beta_2(m{010}) + \beta_3(m_{001}) + \beta_{12}(m_{110}) + \beta_{13}(m_{101}) + \beta_{23}(m_{011})$$
...
When estimation of the total “population” of missing people in Casanare is the goal (as it typically is with multiple-systems estimation), the key value here is the intercept $a$. To estimate $\log(m_{000}$), all the other $m$ values are zero. Therefore the only term that contributes to the estimate of $\log(m_{000}$) is a. The value of $m_{000}$ is therefore the exponentiated value of $a$, that is, $\exp(a)$. The total number of cases is the sum of the observed cases (that is, the seven values of $m$) plus $\exp(a)$.
...
We need to find the best model in order to get an accurate estimate of $a$. Thus, we should determine whether the full (saturated) model above, which assumes that all three two-way interactions between datasets are important, is actually necessary. There are three simpler models that assume only one two-way interaction.

$$\log(m_{000}) = a + \beta_1(m_{100}) + \beta_2(m_{010}) + \beta_3(m_{001}) + \beta_{12}(m_{110})$$

$$\log(m_{000}) = a + \beta_1(m_{100}) + \beta_2(m_{010}) + \beta_3(m_{001}) + \beta_{13}(m_{101})$$

$$\log(m_{000}) = a + \beta_1(m_{100}) + \beta_2(m_{010}) + \beta_3(m_{001}) + \beta_{23}(m_{011})$$

$$\log \left(m_{000}\right)=a+\beta_{1}\left(m_{100}\right)+\beta_{2}\left(m_{010}\right)+\beta_{3}\left(m_{001}\right)+\beta_{12}\left(m_{110}\right)+\beta_{13}\left(m_{101}\right)$$ 

$$\log \left(m_{000}\right)=a+\beta_{1}\left(m_{100}\right)+\beta_{2}\left(m_{010}\right)+\beta_{3}\left(m_{001}\right)+\beta_{12}\left(m_{110}\right)+\beta_{23}\left(m_{011}\right)$$

$$\log \left(m_{000}\right)=a+\beta_{1}\left(m_{100}\right)+\beta_{2}\left(m_{010}\right)+\beta_{3}\left(m_{001}\right)+\beta_{13}\left(m_{101}\right)+\beta_{23}\left(m_{011}\right)$$

In this case, we can clearly write out the seven possible models, when we model selection into three datasets, we estimate all seven of these, and choose the model that minimizes the Bayesian Information Criterion (BIC), a test that weighs goodness of fit against degrees of freedom.


## Challenges with Loglinear models

* Interpretation: The inclusion of so many variables in loglinear models often makes interpretation very difficult.

* Independence: The frequency in each cell is independent of frequencies in all other cells, which is not necessarily the case here. We attempt to model this. 

* Adequate Sample Size: With loglinear models, you need to have at least 5 times the number of cases as cells in your data.  For example, if you have a 2x2x3 table, then you need to have 60 cases.  If you do not have the required amount of cases, then you need to increase the sample size or eliminate one or more of the variables.


## Choosing our "Systems"

Our dataset encompasses 15 lists, far too many to model with a loglinear model. We collapse our 15 lists into 4 systems, 
...
head(cap_indicators %>% dplyr::select(security_ind, forensic_ind, judicial_ind, civil_ind, Freq))
...
## Results of Loglinear Modelling
...
$$\log(\frac{f_i}{t \choose i}) = \log (\frac{N \times P(i \text{ captures})}{t \choose i}) = \log(N(1-p)^{t-i} p^i = \log(N(1-p)^t) + i\log(\frac{p}{1-p})$$
